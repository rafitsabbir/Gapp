package com.sharestream.api_demo;
...
import com.sharestream.api_demo.MainActivity;
import com.sharestream.api_demo.R;
import com.sharestream.api_demo.SettingsDialogTransmitter;
...
public class AnimationView extends View {
	...
	Paint paint, backGround, shapePaint, textPaint; // using this, we can draw
													// on canvas
	...
	public AnimationView(Context context) {
		...
		// textPaint.setARGB(255, 0, 146, 200);
	...
	public void onDraw(Canvas canvas) {
		...
		if (enableDraw) {
			...
			if (!text.equalsIgnoreCase("notext")) {
				...
				LayoutInflater inflater = MainActivity.mainInstance
						.getLayoutInflater();
				View layouttoast = inflater.inflate(R.layout.custom_toast,
						(ViewGroup) findViewById(R.id.custom_toast));
				((TextView) layouttoast.findViewById(R.id.toasttext)).setText(
						text);

				Toast mytoast = new Toast(this.getContext());
				mytoast.setView(layouttoast);
				mytoast.setDuration(Toast.LENGTH_LONG);
				mytoast.setGravity(Gravity.TOP | Gravity.CENTER_HORIZONTAL,
						320, 0);
				mytoast.show();
			}
		} else if (!enableDraw && clear) {
			canvas.drawColor(0, Mode.CLEAR);
			canvas.drawText("Please, tap for menu", 50, 80, textPaint);
			canvas.drawText("and tap \"Share now\"", 50, 140, textPaint);
			canvas.drawText("to select person", 50, 200, textPaint);
			canvas.drawText("and tap to person", 50, 260, textPaint);
			canvas.drawText("to share stream", 50, 320, textPaint);
			clear = false;
		}
	}
